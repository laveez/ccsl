...
import { extractWorktreeName, extractRepoName, extractRepoNameFromCommonDir, getProjectDir, } from "./utils.js";
import { buildStatuslineOutput, readStatuslineConfig } from "./render.js";
const execP = promisify(exec);
// ============================================================================
// Git Functions
// ============================================================================
async function getGitFileStats(projectDir) {
...
async function getAheadBehind(projectDir) {
    try {
...
async function getLinkedWorktreeCommonDir(projectDir) {
    try {
...
export async function fetchGitRepoInfo(projectDir) {
    try {
...
export async function fetchPrInfo() {
    try {
        const { stdout } = await execP("gh pr view --json=number,url,title,isDraft,state,mergeStateStatus");
...
export async function parseTranscriptFull(transcriptPath) {
    const result = {
...
export function countConfigs(cwd) {
    ...
    if (cwd) {
        if (existsSync(join(cwd, "CLAUDE.md")))
            claudeMdCount++;
        if (existsSync(join(cwd, "CLAUDE.local.md")))
            claudeMdCount++;
        if (existsSync(join(cwd, ".claude", "CLAUDE.md")))
            claudeMdCount++;
        if (existsSync(join(cwd, ".claude", "CLAUDE.local.md")))
            claudeMdCount++;
...
function writeUsageCache(data, timestamp) {
    try {
        const cachePath = getUsageCachePath();
...
function isKeychainBackoff(now) {
    try {
...
function recordKeychainFailure(now) {
    try {
        const backoffPath = getKeychainBackoffPath();
...
function readKeychainCredentials(now) {
    if (platform() !== "darwin")
        return null;
    if (isKeychainBackoff(now))
        return null;
    try {
...
function readFileCredentials(now) {
...
// ============================================================================
export function getLearningStatus(sessionStart) {
    const claudeDir = join(homedir(), ".claude");
