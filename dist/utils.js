...
export function fgWhite() {
    return "\x1b[97m";
}
export function fgBlack() {
    return "\x1b[30m";
}
...
export function cleanErrorMessage(str) {
    let cleaned = str.replace(/<[^>]+>/g, "");
    cleaned = cleaned.replace(/\n/g, " ").replace(/\s+/g, " ").trim();
    cleaned = cleaned.replace(/\.\s*Current working directory:\s*\S+/i, "");
    return cleaned;
}
export function colorSegment(bgR, bgG, bgB, text) {
...
export function truncateToWidth(str, maxWidth) {
    if (maxWidth <= 0)
        return reset();
    if (getVisibleWidth(str) <= maxWidth)
        return str;
    let visibleWidth = 0;
    let result = "";
    let i = 0;
    while (i < str.length && visibleWidth < maxWidth) {
        if (str[i] === "\x1b" && i + 1 < str.length) {
            if (str[i + 1] === "[") {
                let j = i + 2;
                while (j < str.length && !/[@A-Z\[\]^_`a-z{|}~]/.test(str[j]))
                    j++;
                if (j < str.length)
                    j++;
...
export function joinSegmentsWithWrap(segments, maxWidth) {
    if (maxWidth <= 0) {
...
export function gradientColor(stops, value) {
    if (value <= stops[0][0])
        return stops[0][1];
...
export function badgeGradient(bg, text) {
...
export function badgeRich(color, text) {
    const [r, g, b] = BADGE[color];
...
export function formatFileStats(stats) {
    if (!stats)
        return "";
    const parts = [];
    if (stats.modified > 0)
    ...
    if (stats.untracked > 0)
        parts.push(`?${stats.untracked}`);
    return parts.join("");
}
export function formatToolCall(toolName, input, cwd) {
    if (!input) {
        return `${toolName}()`;
    }
    const paramMap = {
...
export function makeRelativePath(path, cwd) {
...
export function makeRelativeToPath(path, cwd) {
    const cwdNormalized = cwd.endsWith("/") ? cwd : cwd + "/";
    if (path.startsWith(cwdNormalized)) {
        return path.substring(cwdNormalized.length);
    }
...
export function getShortMcpName(fullName) {
...
export function getToolDisplayName(fullName) {
...
export function extractRepoName(toplevelPath) {
    return toplevelPath.trim().split("/").pop() || "";
}
export function extractRepoNameFromCommonDir(commonDir) {
...
export function extractWorktreeName(toplevelPath) {
    return toplevelPath.trim().split("/").pop() || "";
}
export function getModelName(input) {
...
export function calculatePercentUsed(currentTokens, contextSize) {
    if (contextSize === 0)
        return 0;
    return Math.floor((currentTokens * 100) / contextSize);
}
export function extractTicketMarker(title) {
    const match = title.match(/^([A-Z]{2,6}-\d{1,5})/);
    return match ? match[1] : null;
}
export function getPrStatusSuffix(prInfo) {
    if (prInfo.isDraft)
        return " (D)";
